<template>
  <div class="fixed inset-0 z-99999 flex items-center justify-center p-4 sm:p-6 animate-fade-in">

    <div class="absolute inset-0 bg-[#051021]/80 backdrop-blur-sm cursor-pointer" @click="$emit('close')"></div>

    <div class="bg-[#0A192F] border border-white/10 rounded-4xl overflow-hidden max-w-lg w-full shadow-2xl flex flex-col relative z-10" @click.stop>

      <div class="px-6 py-4 border-b border-white/5 flex items-center justify-between bg-white/2">
        <h3 class="text-base font-bold text-white tracking-wide">Comments</h3>
      ...
      <div class="p-6 flex flex-col w-full">
        ...
        <div v-else class="flex flex-col gap-4 max-h-100 overflow-y-auto custom-scrollbar pr-2 mb-4">
        ...
        <div class="flex items-center gap-3 pt-4 border-t border-white/5">
          ...
          <div class="flex-1 bg-black/20 focus-within:bg-white/3 border border-white/5 focus-within:border-white/10 rounded-full flex items-center pr-1 transition-all shadow-inner">
            <input
                ...
                @keydown.enter="handleEmitSubmit"
                type="text"
...
watch(() => props.isSubmitting, (isNowSubmitting, wasSubmitting) => {
  if (wasSubmitting && !isNowSubmitting && !props.errorMsg) {
    commentInput.value = ''
  }
})
</script>
