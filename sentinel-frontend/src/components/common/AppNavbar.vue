      ...
      <div class="relative w-full group">
        ...
        >
          ...
          <ul v-else-if="searchResults.length > 0" class="max-h-80 overflow-y-auto custom-scrollbar py-2">
            <li v-for="user in searchResults" :key="user.publicId">
              <router-link
              ...
              >
                ...
                <div class="flex flex-col truncate">
                  <span class="text-white text-[13px] font-semibold truncate">{{ user.name }}</span>
                  <span class="text-white/40 text-[11px] truncate">@{{ user.username }}</span>
                </div>
              </router-link>
            </li>
          </ul>
...
watch(textInput, (query) => {
  ...
  debounceTimer = setTimeout(async () => {
    try {
      const response = await api.get('/users/searchAccounts', { params: { query } });
