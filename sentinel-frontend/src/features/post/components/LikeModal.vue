<template>
  <div class="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 animate-fade-in">
    ...
    <div class="bg-[#0A192F] border border-white/10 rounded-4xl overflow-hidden max-w-sm w-full shadow-2xl flex flex-col relative z-10" @click.stop>
      ...
      <div class="p-2 max-h-[60vh] min-h-[200px] overflow-y-auto custom-scrollbar flex flex-col">
        ...
        <template v-else>
          ...
          <div v-for="(item, index) in likers" :key="resolveUser(item).publicId || index" class="flex items-center gap-3 p-3 rounded-2xl hover:bg-white/5 transition-colors cursor-pointer group">

            <img v-if="resolveUser(item).profilePicUrl" :src="resolveUser(item).profilePicUrl" class="w-10 h-10 rounded-full object-cover bg-[#0A192F] border border-white/10 shrink-0"  alt=""/>
            ...
            <div class="flex flex-col flex-1 min-w-0">
              <span class="text-[14px] font-bold text-white/90 group-hover:text-blue-400 transition-colors truncate">
                {{ resolveUser(item).name }}
              </span>
              <span v-if="resolveUser(item).username" class="text-[12px] text-white/40 truncate">
...
const resolveUser = (item: any) => {
  if (!item) return {};
  return item.user || item.userEntity || item;
};
</script>
