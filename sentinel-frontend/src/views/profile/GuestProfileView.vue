            ...
            <template v-else>
              <div v-if="profilePosts.length > 0" class="flex flex-col gap-6 mt-4">
                <FeedPost
                    ...
                    :key="post.id"
                    :post="post"
                    :userEntity-profile="guestProfile || undefined"
...
import { useFriendship } from '@features/friendship';
import { FeedPost, PostPreview } from '@features/post';
import { ConnectionsPanel } from '@features/dashboard';

import type { Post, GuestProfile } from '@shared/types';
...
const { friendshipState, isTogglingFriend, friendButtonText, toggleFriendStatus } = useFriendship(guestProfile)

const canViewContent = computed(() => guestProfile.value?.privacy?.timeline === 'Public' || friendshipState.value === 'friends')
const canViewFriends = computed(() => guestProfile.value?.privacy?.friendsList === 'Public' || friendshipState.value === 'friends')
const canViewDetails = computed(() => guestProfile.value?.privacy?.details === 'Public' || friendshipState.value === 'friends')

const recentPhotos = computed(() => profilePosts.value.map(p => p.image).filter((img): img is string => img !== null).slice(0, 4))

const fetchGuestData = async (publicId: string) => {
  loading.value = true
  try {
    const data = await userService.getGuestProfile(publicId)

    guestProfile.value = data.guestProfile
    profilePosts.value = data.profilePosts

    if (data.friendshipStatus === 'ACCEPTED' || data.friends.length > 0) {
      friendshipState.value = 'friends'
      friends.value = data.friends
      mutualFriends.value = data.friends // <-- Populate mutual friends here
    } else {
      friendshipState.value = data.friendshipStatus === 'PENDING' ? 'pending' : 'none'
      friends.value = []
      mutualFriends.value = []
    }

  } catch (err: any) {
    if (err.response && [301, 302, 304].includes(err.response.status)) {
      router.push('/profile')
    } else {
...
onMounted(() => {
  if (route.params.publicId) fetchGuestData(route.params.publicId as string)
})

watch(() => route.params.publicId, (newId) => {
  if (newId) {
    guestProfile.value = null
    fetchGuestData(newId as string)
  }
})
...
const viewImage = (type: 'avatar' | 'banner') => {
  closeSubmenu()
  const url = type === 'avatar' ? guestProfile.value?.profilePicUrl : guestProfile.value?.bannerPicUrl
  if (url) previewImage.value = { url, type }
}
